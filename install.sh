...
yellow='\033[0;33m'
plain='\033[0m'
software=(Docker_Caddy-使用自签证书的WSS模式 Docker_Caddy_cloudflare-使用CF证书的WSS模式 Docker-TCP模式且同时支持WS模式)
operation=(install-全新安装 update_config-更新配置 update_image-更新镜像 logs-查看日志)
...
pre_install_docker_compose(){
    # Set ssrpanel_url
    echo "Please ssrpanel_url-输入你的sspanel网址"
    read -p "(http://xxx.com 有tls的换成https 注意网址最后不要有斜杠‘/’):" ssrpanel_url
    [ -z "${ssrpanel_url}" ]
    echo
    echo "---------------------------"
    echo "ssrpanel的网址 = ${ssrpanel_url}"
    echo "---------------------------"
    echo
    # Set ssrpanel key
    ...
    echo
    echo "---------------------------"
    echo "ssrpanel通讯密钥 = ${ssrpanel_key}"
    echo "---------------------------"
    echo

    # Set ssrpanel speedtest function
    echo "你打算隔多久用speedtest进行一次节点网速测试"
    read -p "(ssrpanel speedtest: 回车默认6小时进行一次):" ssrpanel_speedtest
    ...
    echo
    echo "---------------------------"
    echo "ssrpanel节点ID = ${ssrpanel_node_id}"
    echo "---------------------------"
    echo
    ...
    echo
    echo "---------------------------"
    echo "v2ray前端面板类型 = ${v2ray_downWithPanel}"
    echo "---------------------------"
    echo
}
...
pre_install_caddy(){
    ...
    echo "输入你解析到本节点服务器的域名"
    read -p "(WS模式要求提供解析到本节点IP的网址):" v2ray_domain
    [ -z "${v2ray_domain}" ]
    echo
    echo "---------------------------"
    echo "v2ray伪装域名 = ${v2ray_domain}"
    echo "---------------------------"
    echo
    ...
    read -p "(务必与前端节点信息的Path值相同,回车默认: /v2ray):" v2ray_path
...
config_docker(){
    ...
    echo "等待加载docker配置文件"
    curl -L https://raw.githubusercontent.com/GODDOG-1/xxoo/master/Docker/V2ray/docker-compose.yml > docker-compose.yml
...
config_caddy_docker(){
    ...
    echo "开始安装软件包"
    install_dependencies
    curl -L https://raw.githubusercontent.com/GODDOG-1/xxoo/master/Docker/Caddy_V2ray/Caddyfile >  Caddyfile
    echo "等待加载docker配置文件"
    curl -L https://raw.githubusercontent.com/GODDOG-1/xxoo/master/Docker/Caddy_V2ray/docker-compose.yml > docker-compose.yml
    sed -i "s|node_id:.*|node_id: ${ssrpanel_node_id}|"  ./docker-compose.yml
    sed -i "s|sspanel_url:.*|sspanel_url: '${ssrpanel_url}'|"  ./docker-compose.yml
    sed -i "s|key:.*|key: '${ssrpanel_key}'|"  ./docker-compose.yml
    sed -i "s|speedtest:.*|speedtest: ${ssrpanel_speedtest}|"  ./docker-compose.yml
...
install_docker(){
    ...
    echo "4、玄学问题 你太帅导致的 指导联系TG@FakeloveMix"
    echo
}
...
install_select(){
    ...
    while true
    do
    echo  "你选了哪个docker的安装方式:"
    for ((i=1;i<=${#software[@]};i++ )); do
        hint="${software[$i-1]}"
        echo -e "${green}${i}${plain}) ${hint}"
    done
    read -p "Please enter a number (回车默认 ${software[0]}):" selected
...
install_dependencies(){
    ...
    echo -e "[${green}Info${plain}] 将时区设置为上海东八区"
